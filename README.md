...
- **7 preset themes** — Everforest Light/Dark, Rosé Pine Dawn, Dracula, Gruvbox, Nord, GitHub
- **Adaptive mode** — AI designs custom HTML/CSS tailored to your content
- **Fixed width (750px) + auto height** — mobile-friendly, content determines card height
- **2x Retina** output by default
- **Cross-platform** — Windows (Edge), macOS (Chrome), Linux (Chromium)
- **No bundled Chromium** — uses your system browser, ~10 MB total footprint
...
Then in Claude Code, say: "share", "export", "make a card", "生成图片".
...
- **Fixed mode**: Agent replaces `[CONTENT]` placeholder in a chosen template, renders via `puppeteer-core`
- **Adaptive mode**: Agent reads content, designs a custom HTML page from scratch, free color choice (Catppuccin palette available as reference)
- **Renderer**: Opens system Edge/Chrome in headless mode, screenshots `#card-wrapper` element at 750px width

## Requirements

- Node.js ≥ 18
- Edge / Chrome / Chromium (system browser, auto-detected)
